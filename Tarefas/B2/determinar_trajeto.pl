...
resultado(Metodo) :-
    (Metodo == 'mais_rapido', restaurant(_,LE,_), random_member(E,LE), write('ENTREGAR PARA:'),write(E),nl,nl, caminho_mais_rapido(restaurante, E));
    (Metodo == 'hillclimbing', restaurant(_,LE,_), random_member(E,LE), write('ENTREGAR PARA:'),write(E),nl,nl, demo(restaurante, E,_)).
